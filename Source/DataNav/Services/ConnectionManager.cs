...
public class ConnectionManager
{
    ...
    public IDbConnection AddConnection(ConnectionInfo connectionInfo)
    {
        ...
        Console.WriteLine($"Creating connection for {connectionInfo.Name} with provider {connectionInfo.ProviderType}");

        var connection = factory(connectionInfo);
        _connections[connectionInfo.Name] = connection;

        // Raise event
        Console.WriteLine($"About to raise ConnectionAdded event for {connectionInfo.Name}");
        ConnectionAdded?.Invoke(this, new ConnectionEventArgs(connection));
        Console.WriteLine($"ConnectionAdded event raised for {connectionInfo.Name}");
